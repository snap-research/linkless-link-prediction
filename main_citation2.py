...
def train(model, predictor, teacher_model, teacher_predictor, data, split_edge, optimizer, batch_size, args, device):
    model.train()
    predictor.train()
    ...
    for perm in DataLoader(range(source_edge.size(0)), batch_size,
        ...
        node_perm = next(node_loader).to(device)
...
def main():
    ...
    pretrained_model = torch.load("saved_models/citation2-sage.pkl", device)
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            loss = train(model, predictor, teacher_model, teacher_predictor, data, split_edge, optimizer,
